#!/usr/bin/env python3
"""
BlackBox SIH26189 Large-Scale Synthetic Dataset Generator
Generates 20,000+ synthetic records for criminal network analysis demonstration
"""
...
random.seed(42)
np_random = random  # Using random module for simplicity
...
class SyntheticWorldGenerator:
    def __init__(self):
        ...
        self.first_names_male = [
            ...
            'Vijay', 'Anil', 'Suresh', 'Rajiv', 'Ashish', 'Nitin', 'Jitendra'
        ]
    ...
    def generate_vehicle_registration(self):
        ...
        # Format: XX 00 XX 0000 or similar
        ...
        series_chars = ''.join([chr(random.randint(65, 90)) for _ in range(2)])  # AA-ZZ
        number = f"{random.randint(1, 9999):04d}"
        return f"{state_code} {district_num} {series_chars} {number}"
    
    def generate_person(self, gender=None, age_range=(20, 60)):
        """Generate a synthetic person entity"""
        ...
        nicknames = []
        if len(first_name) > 3:
            nicknames.append(first_name[:3])  # First 3 letters
            if len(first_name) > 4:
                nicknames.append(first_name[:4])  # First 4 letters
        ...
        # Add nickname from nicknames list
        if nicknames:
            aliases.append(random.choice(nicknames))
        
        age = random.randint(age_range[0], age_range[1])
        
        # Generate contact information
        phone_ids = []
        # Each person has 1-3 phone numbers
        for _ in range(random.randint(1, 3)):
            phone_id = self.generate_phone_entity()
            phone_ids.append(phone_id)
        
        # Generate address(es)
        location_ids = []
        # Each person has 1-2 addresses (current, maybe previous)
        for _ in range(random.randint(1, 2)):
            location_id = self.generate_location_entity()
            location_ids.append(location_id)
        
        # Generate vehicle ownership
        vehicle_ids = []
        # Each person has 0-2 vehicles
        for _ in range(random.randint(0, 2)):
            vehicle_id = self.generate_vehicle_entity()
            vehicle_ids.append(vehicle_id)
        
        # Generate organization associations
        organization_ids = []
        # Each person is associated with 0-2 organizations
        for _ in range(random.randint(0, 2)):
            org_id = self.generate_organization_entity()
            organization_ids.append(org_id)
        
        # Generate bank accounts
        bank_account_ids = []
        # Each person has 1-3 bank accounts
        for _ in range(random.randint(1, 3)):
            account_id = self.generate_bank_account_entity()
            bank_account_ids.append(account_id)
        ...
        person_entity = {
            ...
            'addresses': location_ids,
            'vehicles': vehicle_ids,
            'organizations': organization_ids,
            'bank_accounts': bank_account_ids,
            'date_added': datetime.now().isoformat(),
            'validity_period': {
                'start': (datetime.now() - timedelta(days=random.randint(365, 1825))).isoformat(),
                'end': (datetime.now() + timedelta(days=random.randint(365, 1825))).isoformat()
            }
        }
    ...
    def generate_phone_entity(self):
        """Generate a synthetic phone number entity"""
        phone_id = self.generate_id('phone_numbers')
        formatted_number, raw_number = self.generate_phone_number()
        
        # Determine if it's a burner/disposable phone (10% chance)
        is_burner = random.random() < 0.1
        # Determine if it's a landline (5% chance)
        is_landline = random.random() < 0.05 and not is_burner
    ...
    def generate_vehicle_entity(self):
        """Generate a synthetic vehicle entity"""
        ...
        vehicle_entity = {
            ...
            'is_active': random.random() > 0.05,  # 95% active (some may be scrapped/sold)
            'date_added': datetime.now().isoformat(),
            'validity_period': {
    ...
    def generate_location_entity(self):
        """Generate a synthetic location entity"""
        location_id = self.generate_id('locations')
        city = random.choice(self.cities)
        state = random.choice([s for s in self.states if s in ['Maharashtra', 'Delhi', 'Karnataka', 'Telangana', 'Gujarat', 'Tamil Nadu', 'West Bengal']] or self.cities)  # Bias toward major states
        # Ensure state matches city approximately (simplified)
        ...
        address_formats = [
            ...
            f"{building_num}/{random.randint(1, 50)}, {street}, {area}, {city} - {random.randint(100000, 999999)}",
            f"No. {building_num}, {street}, {area}, {city} - {random.randint(100000, 999999)}",
            f"{building_num}-{random.randint(1, 20)}, {street}, {area}, {city} - {random.randint(100000, 999999)}",
    ...
    def generate_organization_entity(self):
        ...
        # Generate location(s)
        location_ids = []
        # Each organization has 1-3 locations
        for _ in range(random.randint(1, 3)):
            location_id = self.generate_location_entity()
            location_ids.append(location_id)
        ...
        phone_ids = []
        # Each organization has 1-3 phone numbers
        for _ in range(random.randint(1, 3)):
            phone_id = self.generate_phone_entity()
            phone_ids.append(phone_id)
    ...
    def generate_bank_account_entity(self):
        """Generate a synthetic bank account entity"""
    ...
    def generate_criminal_network(self, network_type=None, size=None):
        ...
        if size is None:
            # Different network types have different typical sizes
            size_ranges = {
                'hierarchical': (8, 25),
                'hub_and_spoke': (6, 15),
                'loosely_connected': (10, 30),
                'clustered': (12, 35),
                'decentralized': (8, 20),
                'pipeline': (5, 12),
                'distribution': (10, 25),
                'corruption': (5, 15),
                'terror_financing': (4, 10),
                'wildlife_poaching': (6, 18)
            }
            min_size, max_size = size_ranges.get(network_type, (8, 20))
            size = random.randint(min_size, max_size)
        
        # Generate members for this network
        member_ids = []
        for i in range(size):
            person_id = self.generate_person(age_range=(22, 55))  # Criminals tend to be in this age range
        ...
        network_entity = {
            ...
            'date_added': datetime.now().isoformat(),
            'active_period': {
                'start': (datetime.now() - timedelta(days=random.randint(180, 730))).isoformat(),
    ...
    def generate_case(self, case_type=None, primary_network=None):
        ...
        investigating_officer = self.generate_person(age_range=(30, 55)) if random.random() < 0.8 else None
        supervising_officer = self.generate_person(age_range=(35, 60)) if random.random() < 0.6 else None
    ...
    def _get_state_abbreviation(self):
        state_abbrevs = {
            ...
            'Odisha': 'OR', 'Kerala': 'KL', 'Jharkhand': 'JH', 'Assam': 'AS', 'Punjab': 'PB',
    ...
    def generate_synthetic_world(self):
        """Generate the foundational synthetic world"""
        print("Generating synthetic world...")
        
        # Generate people (1200 total)
        ...
        # Generate phone numbers (600 total)
        ...
        # Generate organizations (180 total)
        ...
        # Generate bank accounts (350 total)
