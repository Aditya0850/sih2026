#!/usr/bin/env python3
"""
BlackBox SIH26189 Large-Scale Synthetic Dataset Generator - CLEAN VERSION
...
class SyntheticWorldGenerator:
    ...
    def generate_person(self, gender=None, age_range=(20, 60)):
        ...
        phone_ids = []
        # Each person has 1-2 phone numbers (reduced for control)
        ...
        organization_ids = []
        # Each person is associated with 0-1 organizations
        if random.random() < 0.4:  # 40% associated with an organization
            org_id = self.generate_organization_entity()
            organization_ids.append(org_id)
        
        # Generate bank accounts
        bank_account_ids = []
        # Each person has 1-2 bank accounts
        for _ in range(random.randint(1, 2)):
    ...
    def generate_organization_entity(self):
        ...
        # Generate location(s)
        location_ids = []
        # Each organization has 1-2 locations
        for _ in range(random.randint(1, 2)):
            location_id = self.generate_location_entity()
            location_ids.append(location_id)
        ...
        phone_ids = []
        # Each organization has 1-2 phone numbers
        for _ in range(random.randint(1, 2)):
            phone_id = self.generate_phone_entity()
            phone_ids.append(phone_id)
