...
async def upload_evidence(
    ...
    case_ids: Optional[str] = Form(default=None, description="Comma-separated list of case IDs (UUIDs) to link the evidence to. Provide a single UUID or multiple UUIDs separated by commas."),
    ...
    parsed_case_ids: list[UUID] = []
    if case_ids:
        # Handle both single UUID string and comma-separated UUID strings
        case_id_strings = [cid.strip() for cid in case_ids.split(",") if cid.strip()]
        for case_id_str in case_id_strings:
            try:
                case_id = UUID(case_id_str)
                parsed_case_ids.append(case_id)
            except ValueError:
                # Skip invalid UUIDs
                continue
    ...
    if parsed_case_ids:
        ...
        for case_id in parsed_case_ids:
    ...
    # Trigger evidence processing pipeline after successful upload
    try:
        ...
        orchestrator = PipelineOrchestrator(stages, pipeline_version="1.0.0")
        
        # Create evidence context
        context = EvidenceContext(
            evidence_id=str(created_evidence.id),
            snapshot_id=str(uuid4()),  # placeholder snapshot ID
        ...
        logger.info(f"Pipeline triggered for evidence {created_evidence.id}")
    except Exception as e:
        logger.error(f"Pipeline failed for evidence {created_evidence.id}: {e}", exc_info=True)
